    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetContent side={isRTL ? 'right' : 'left'} className='w-[250px] sm:w-[300px] p-0 flex flex-col [&>button]:hidden'>
        ...
        <div className='p-4 border-t bg-muted/30 space-y-2.5 flex-shrink-0'>
          ...
          <div className='flex items-center justify-between px-4 py-2 bg-background rounded-xl'><div className='flex items-center gap-2'><Settings className='h-4 w-4 text-muted-foreground' /><span className='text-sm font-semibold'>{language === 'ar' ? 'المظهر' : 'Theme'}</span></div><ThemeToggle /></div>
          {isAuthenticated ? (
